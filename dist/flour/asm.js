...
class FlourAssembler {
    static assemble(assembler_text) {
        const parsed = assembler_text.trim().split("\n").map(FlourAssembler.parse_expression);
        const processed = FlourAssembler.process_labels(parsed);
        const bytes = processed.map(FlourAssembler.bytify_expression);
        const merged = new Uint8Array(bytes.length * instructionLength);
    ...
    static bytify_data(data, address_mode) {
        const data_bytes = new Uint8Array(4);
        if (address_mode == "register") {
            ...
            const bits = (parseInt(data) >>> 0).toString(2);
            if (bits.length > 32) {
                throw 'Integer overflow';
            }
            const padded = bits.padStart(32, '0');
            console.log(padded);
    ...
    static bytify_expression(expr) {
        const bytes = new Uint8Array(instructionLength);
        let byteIndex = 0;
        if (typeof expr[0] != 'string') {
            throw 'format error';
        }
        const operation = expr[0];
    ...
    static process_labels(expressionList) {
        const labelMap = new Map();
        let index = 0;
        const proccessedExpressionList = [];
...
exports.FlourAssembler = FlourAssembler;
